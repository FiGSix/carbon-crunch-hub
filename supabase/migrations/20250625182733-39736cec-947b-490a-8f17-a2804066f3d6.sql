...
-- STEP 1: Clean up all existing RLS policies to prevent conflicts
DO $$
BEGIN
    -- Drop all existing policies on profiles with CASCADE to handle dependencies
...
-- STEP 2: Recreate security definer functions to prevent recursion
...
-- PROFILES TABLE POLICIES (fixed to prevent infinite recursion)
...
FOR SELECT USING (
  EXISTS(SELECT 1 FROM public.profiles p WHERE p.id = auth.uid() AND p.role IN ('agent', 'admin')) OR 
  auth.uid() = user_id
);
...
-- STEP 4: Create performance indexes for better query optimization
