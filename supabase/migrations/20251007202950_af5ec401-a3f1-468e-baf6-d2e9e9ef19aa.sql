...
-- The issue: invitation_token and invitation_expires_at are both RETURNS TABLE output parameters
-- and column names, causing ambiguity in unqualified references
...
BEGIN
  -- Fixed: Added table alias 'p' to avoid ambiguity with RETURNS TABLE parameters
  SELECT 
    EXISTS(
      SELECT 1 
      FROM proposals p
