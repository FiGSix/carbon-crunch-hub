-- Fix get_proposal_by_token_direct to remove non-existent columns
CREATE OR REPLACE FUNCTION public.get_proposal_by_token_direct(token_param text)
 RETURNS TABLE(
   ...
   review_later_until timestamp with time zone, 
   is_preview boolean, 
   preview_of_id uuid, 
   client_email text, 
   ...
   client_share_percentage numeric
 )
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Get client email with proper aliasing
  SELECT 
    COALESCE(
...
-- Fix get_proposal_by_token to remove non-existent columns
