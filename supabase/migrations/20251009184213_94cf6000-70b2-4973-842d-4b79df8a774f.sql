-- Fix client search to allow agents to see all clients
...
BEGIN
  ...
  WITH deduplicated_clients AS (
    ...
    WHERE (
      ...
      -- Allow admins and agents to see all clients
      AND (
        current_user_role IN ('admin', 'agent')
...
-- Update RLS policies on clients table to allow agents to view and update all clients

-- Drop existing restrictive policies
DROP POLICY IF EXISTS "Agents can view clients they created" ON clients;
DROP POLICY IF EXISTS "Users can update own clients" ON clients;

-- Create new policy that allows agents to view all clients
CREATE POLICY "Agents and admins can view all clients"
ON clients FOR SELECT
USING (
  is_current_user_admin() 
  OR is_current_user_agent()
  OR user_id = auth.uid()
);

-- Create new policy that allows agents to update any client record
CREATE POLICY "Agents and admins can update all clients"
