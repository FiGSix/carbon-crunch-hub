...
-- Add index for better query performance on non-deleted profiles
CREATE INDEX IF NOT EXISTS idx_profiles_deleted_at 
ON public.profiles(deleted_at) 
WHERE deleted_at IS NULL;

-- Update the get_agents_management_data function to exclude soft-deleted agents
...
BEGIN
  ...
  AND pr.deleted_at IS NULL  -- Exclude soft-deleted agents
