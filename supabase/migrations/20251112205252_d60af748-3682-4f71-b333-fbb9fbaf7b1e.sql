-- Fix commission_date extraction in get_dashboard_metrics_by_stage
-- The column p.commission_date doesn't exist - it's stored in JSONB fields
...
CREATE OR REPLACE FUNCTION public.get_dashboard_metrics_by_stage(user_id_param uuid, user_role_param text)
 RETURNS TABLE(audit_ready_mwp numeric, audit_ready_revenue numeric, audit_review_requests bigint, onboarding_mwp numeric, pending_approval_mwp numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  WITH accessible_proposals AS (
    ...
    SELECT 
      ...
      p.status,
      p.signed_at,
      -- Extract commission_date from JSONB with fallbacks
      COALESCE(
        (p.project_info->>'commissionDate')::DATE,
        (p.content->'projectInfo'->>'commissionDate')::DATE,
        p.signed_at::DATE,
        CURRENT_DATE
      ) as commission_date,
