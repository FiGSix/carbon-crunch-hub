...
-- Create updated function that includes invited agents
...
BEGIN
  ...
  WHERE (status_filter IS NULL OR agent_status = status_filter)
  AND (
    search_term IS NULL OR 
    agent_email ILIKE '%' || search_term || '%' OR
    agent_name ILIKE '%' || search_term || '%' OR
    company_name ILIKE '%' || search_term || '%'
  )
  ORDER BY 
    CASE WHEN agent_status = 'pending_approval' THEN 0 ELSE 1 END,
    CASE WHEN agent_status = 'invited' THEN 0 ELSE 1 END,
