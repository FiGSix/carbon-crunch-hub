-- Update get_dashboard_metrics_by_stage to include sent, delivered, opened, stale in pending approval metrics
...
BEGIN
  ...
  metrics AS (
    SELECT
      ...
      -- Card 5: Pending Approval (all unsigned proposals awaiting client action)
      -- Includes: pending, draft, sent, delivered, opened, stale
      COALESCE(
        SUM(CASE 
