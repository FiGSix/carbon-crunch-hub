-- Restore Pending Approval Metrics Logic
-- This migration fixes the "Pending Approval" metrics that were incorrectly changed
-- in migration 20260123121646. The original logic should show UNSIGNED proposals
-- awaiting client signature, not signed proposals awaiting admin validation.
...
BEGIN
  ...
  WITH accessible_proposals AS (
    SELECT 
      ...
      p.agent_share_percentage,
      p.admin_share_percentage,
      p.agent_id
    FROM proposals p
      ...
      SUM(CASE 
        ...
        THEN 
          CASE user_role_param
            ...
            WHEN 'agent' THEN 
              carbon_credits * 95 * 6 * COALESCE(agent_share_percentage, 4) / 100
            WHEN 'admin' THEN 
              carbon_credits * 95 * 6 * COALESCE(admin_share_percentage, 26) / 100
            ELSE 0
          END
        ELSE 0 
      END), 
