-- Make apply_referral_on_signup idempotent and prevent duplicate pending link requests

CREATE UNIQUE INDEX IF NOT EXISTS super_partner_link_requests_pending_unique
  ON public.super_partner_link_requests (super_partner_id, company_id, request_type)
...
BEGIN
  ...
  -- Idempotency guard: has this user already been recorded as a signup for this link?
  SELECT EXISTS (
  ...
  -- Attribute profile (idempotent — same values on re-run)
