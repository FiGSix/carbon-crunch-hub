CREATE OR REPLACE FUNCTION public.get_agents_management_data(status_filter text DEFAULT NULL::text, search_term text DEFAULT NULL::text, limit_param integer DEFAULT 50, offset_param integer DEFAULT 0)
 RETURNS TABLE(agent_id uuid, agent_name text, agent_email text, company_name text, company_id uuid, company_commission_override numeric, company_signed_kwp numeric, agent_status text, access_level text, commission_override numeric, last_active_at timestamp with time zone, total_proposals bigint, active_proposals bigint, signed_proposals bigint, total_commission numeric, join_date date, onboarding_completed boolean, portfolio_size_kwp numeric, is_invitation boolean, invitation_id uuid, invitation_token text, invitation_expires_at timestamp with time zone, invited_by_email text)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  solo_company AS (
    -- Fallback: user created a company but has no active membership row
  ...
  sp_inherit AS (
    -- Rate inherited from the inviting super-partner (via agent_invitations by email)
